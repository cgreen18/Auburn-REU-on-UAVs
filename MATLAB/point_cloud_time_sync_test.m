...
total_translation_matrix = cell(1,N_Frames); 
%position millimeter to meter converstion
conversion_factor = 10^3; 

for ii = 1:N_Frames
    eul = differenceFrames(ii,1:3); %convert to radians
    rotation_from_euler{ii} = horzcat(vertcat(euler2rot(eul),[0 0 0]),[0 0 0 1]'); 
    isRigid(affine3d(rotation_from_euler{ii}))
    translations_from_positions{ii} = horzcat(vertcat(eye(3),[0 0 0]),[differenceFrames(ii,4)/conversion_factor differenceFrames(ii,5)/conversion_factor differenceFrames(ii,6)/conversion_factor 1]');
    total_translation_matrix{ii} = affine3d((rotation_from_euler{ii}*translations_from_positions{ii})');
...
mergeSize = 0.0012; %determines how much of every frame is kept after merge, smaller = more iterations = more points
...
% % % %     tform = pcregistericp(moving, fixed, 'Metric','pointToPoint','MaxIterations',40,'Tolerance',[0.00001, 0.00005],'InitialTransform',init_transform);
...
function rot_matrix = euler2rot(angles)
    phi = angles(3) *pi/180; theta = angles(2) *pi/180; psi = angles(1) *pi/180; 
